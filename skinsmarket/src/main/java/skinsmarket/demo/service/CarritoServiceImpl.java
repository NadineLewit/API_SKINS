...
public class CarritoServiceImpl implements CarritoService {
    ...
    private void validarVendedorListoParaCobrar(Skin skin) {
        User vendedor = skin.getVendedor();
        if (vendedor == null) return;

        boolean faltaTradeUrl = !TradeProfileValidator.isValidTradeUrl(vendedor.getTradeUrl());
        boolean faltaAlias = vendedor.getAliasCobro() == null || vendedor.getAliasCobro().isBlank();
        if (faltaTradeUrl || faltaAlias) {
            throw new RuntimeException(
                    "Esta publicación no está disponible para compra hasta que el vendedor configure su Steam Trade URL y alias de cobro.");
        }
    }
