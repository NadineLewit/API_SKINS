...
public class SkinController {
    ...
    @PostMapping("/admin/create-for-user")
    public ResponseEntity<?> createSkinForUser(
            @RequestParam String email,
            @RequestBody SkinRequest request) {
        try {
            Skin result = skinService.createSkinForUser(email, request);
            return ResponseEntity.status(201)
                    .body(ApiResponse.of("Skin creada y asignada al vendedor " + email, result));
        } catch (NegativeStockException | NegativePriceException | InvalidDiscountException e) {
            return ResponseEntity.badRequest().body(ApiResponse.of(e.getMessage()));
        } catch (RuntimeException e) {
            return ResponseEntity.badRequest().body(ApiResponse.of(e.getMessage()));
            ...
            return ResponseEntity.internalServerError()
                    .body(ApiResponse.of("Error al crear la skin para el usuario: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> inactivarSkin(@PathVariable Long id) {
        try {
            ...
            if (!deleted) {
                return ResponseEntity.status(404)
                        .body(ApiResponse.of("Publicación no encontrada con id: " + id));
            }
            return ResponseEntity.ok(ApiResponse.of("Publicación eliminada exitosamente"));
        } catch (RuntimeException e) {
            return ResponseEntity.badRequest().body(ApiResponse.of(e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> activarSkin(@PathVariable Long id) {
        try {
            boolean activated = skinService.activateSkin(id);
            if (!activated) {
                return ResponseEntity.status(404)
                        .body(ApiResponse.of("Skin no encontrada con id: " + id));
            }
            return ResponseEntity.ok(ApiResponse.of("Skin activada exitosamente"));
        } catch (RuntimeException e) {
            return ResponseEntity.badRequest().body(ApiResponse.of(e.getMessage()));
        }
    }
    ...
    @GetMapping("/mis-skins/historial")
    public ResponseEntity<?> historialMisSkins(Authentication auth) {
        List<Skin> skins = skinService.getHistorialSkinsByOwner(auth.getName());
        return ResponseEntity.ok(
                ApiResponse.of("Historial de tus publicaciones (" + skins.size() + ")", skins));
    }
    ...
    public ResponseEntity<?> activarMiSkin(Authentication auth, @PathVariable Long id) {
        try {
            boolean activated = skinService.activateSkinAsVendedor(id, auth.getName());
            if (!activated) {
                return ResponseEntity.status(404)
                        .body(ApiResponse.of("Skin no encontrada con id: " + id));
            }
